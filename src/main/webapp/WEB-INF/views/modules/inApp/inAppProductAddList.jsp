...
<body>
	...
	<ul class="nav nav-tabs">
		<li>
			<a href="${ctx}/product/inAppPurchase/formView?id=${product.specialId}">内购基本信息</a>
		</li>
		<li><a href="${ctx}/product/inAppPurchase/selectProduct?specialId=${product.specialId}">内购商品查看</a></li>
		<c:choose>
			<c:when test="${product.specialId!=null && not empty product.specialId}">
			<li class="active"><a href="${ctx}/product/inAppPurchase/productList?specialId=${product.specialId}">内购商品添加</a></li>
			</c:when>
			<c:otherwise>
	...
	<form:form id="searchForm" modelAttribute="product" action="${ctx}/product/inAppPurchase/productList" method="post" class="breadcrumb form-search">
...
<!-- 			</li> -->
			<li><label>商品分类：</label> <sys:treeselect id="productCategory" name="productCategoryId" value="${product.productCategoryId}" labelName="productCategory.name"
					labelValue="${product.productCategory.name}" title="商品分类" allowClear="true" url="/product/productCategory/treeData" module="product" notAllowSelectRoot="false" cssClass="input-small" /></li>
