...
<body>
	...
	<ul class="nav nav-tabs">
		<li>
			<a href="${ctx}/product/inAppPurchase/updateFormView?id=${product.specialId}&updateFlag=${product.updateFlag}">内购基本信息</a>
		</li>
		...
		<c:choose>
			...
			<li class="active"><a href="${ctx}/product/inAppPurchase/updateProductList?specialId=${product.specialId}&updateFlag=${product.updateFlag}">内购商品添加</a></li>
			</c:when>
			<c:otherwise>
	...
	<form:form id="searchForm" modelAttribute="product" action="${ctx}/product/inAppPurchase/updateProductList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			<li><label>商品名称：</label>
				<form:input id="name" path="name" htmlEscape="false" maxlength="255" class="input-medium"/>
			</li>
			<li><label>商品编号：</label>
				<form:input id="productNo" path="productNo" htmlEscape="false" maxlength="255" class="input-medium"/>
			</li>
<!-- 			<li><label>搜索关键词：</label> -->
<%-- 				<form:input id="keyword" path="keyword" htmlEscape="false" maxlength="255" class="input-medium"/> --%>
<!-- 			</li> -->
			<li><label>商品分类：</label> <sys:treeselect id="productCategory" name="productCategoryId" value="${product.productCategoryId}" labelName="productCategory.name"
					labelValue="${product.productCategory.name}" title="商品分类" url="/product/productCategory/treeData" module="product" allowClear="true" notAllowSelectRoot="false" cssClass="input-small" /></li>
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/></li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
	<sys:message content="${message}"/>
	<form:form id="inputForm" action="${ctx}/product/inAppPurchase/editProduct" method="post" class="breadcrumb form-search">
