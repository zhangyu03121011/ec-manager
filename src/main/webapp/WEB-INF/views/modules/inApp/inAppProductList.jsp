...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/product/inAppPurchase/selectProduct?specialId=${product.specialId}">内购商品查看</a></li>
		<c:choose>
	...
	<form:form id="searchForm" modelAttribute="searchProductRef" action="${ctx}/product/inAppPurchase/selectProduct" method="post" class="breadcrumb form-search">
...
<!-- 			</li> -->
			<li><label>商品分类：</label> <sys:treeselect id="productCategory" name="product.productCategoryId" value="${searchProductRef.product.productCategoryId}" labelName="product.productCategory.name"
					labelValue="${searchProductRef.product.productCategory.name}" title="商品分类" url="/product/productCategory/treeData" module="searchProductRef" notAllowSelectRoot="false" allowClear="true" cssClass="input-small" /></li>
		...
		<c:forEach items="${page.list}" var="specialRef">
			<tr>
				...
				<shiro:hasPermission name="product:special:edit"><td>
					<a href="${ctx}/product/inAppPurchase/deleteSpecialProduct?id=${specialRef.id}&&specialId=${specialRef.specialId}" onclick="return confirmx('确认要删除该内购商品吗？', this.href)">删除</a>
