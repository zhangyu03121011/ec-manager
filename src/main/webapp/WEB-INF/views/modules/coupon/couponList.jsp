<%@ page contentType="text/html;charset=UTF-8" import="com.common.ecmanager.common.enums.*" import="java.util.Date"%>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="coupon">
			<tr>
				...
				<td>${coupon.totalSum}</td>
				<td>${coupon.noUserSum}</td>
				<%-- <td>
					${coupon.sum}
				</td> --%>
				<td>
				...
				<td>
					<c:choose>
						<c:when test="${coupon.presentType == '1'}">
								购买满减
						</c:when>
						<c:otherwise>
						</c:otherwise>
				...
				<td>
				   <c:if test="${coupon.couponRange == '1'}">指定专题 </c:if>
				   <c:if test="${coupon.couponRange == '2'}">指定商品</c:if>
				   <c:if test="${coupon.couponRange == '3'}">全场通用</c:if>
				</td>
				<td>
				...
				<c:set var="beginDate">  
				   <fmt:formatDate value="${coupon.beginDate}" pattern="yyyy-MM-dd HH:mm:ss" type="date"/>
				</c:set>
				<c:set var="endDate">  
				   <fmt:formatDate value="${coupon.endDate}" pattern="yyyy-MM-dd HH:mm:ss" type="date"/>
				</c:set>
    				<c:choose>
    					<%-- <c:when test="${coupon.sum > 0 && beginDate <= nowDate && endDate >= beginDate}"> --%>
    					<c:when test="${coupon.sum > 0}">
